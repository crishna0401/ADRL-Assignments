...
class Discriminator(nn.Module):
    ...
    def __init__(self,num_in_channels,filters):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(num_in_channels,filters,4,stride=2,padding=1,bias=False),nn.InstanceNorm2d(filters,affine=True))
        # Input: Batch_size x filters x 64 x 64
        self.conv2 = nn.Sequential(nn.Conv2d(filters,filters*2,4,stride=2,padding=1,bias=False),nn.InstanceNorm2d(filters*2,affine=True))
        # Input: Batch_size x filters*2 x 32 x 32
        self.conv3 = nn.Sequential(nn.Conv2d(filters*2,filters*4,4,stride=2,padding=1,bias=False),nn.InstanceNorm2d(filters*4,affine=True))
        # Input: Batch_size x filters*4 x 16 x 16
        self.conv4 = nn.Sequential(nn.Conv2d(filters*4,filters*8,4,stride=2,padding=1,bias=False),nn.InstanceNorm2d(filters*8,affine=True))
        # Input: Batch_size x filters*8 x 8 x 8
        self.conv5 = nn.Sequential(nn.Conv2d(filters*8,filters*16,4,stride=2,padding=1,bias=False),nn.InstanceNorm2d(filters*16,affine=True))
...
for epoch in range(epochs):
    for i,(x,_) in enumerate(train_loader):
        ...
        for _ in range(CRITIC_ITERATIONS):
            ...
            gen_imgs = gen(z)
            dis.zero_grad()
            dis_real_loss = torch.mean(dis(x).view(-1))
            dis_fake_loss = torch.mean(dis(gen_imgs).view(-1))
            gp = gradient_penalty(dis,x,gen_imgs,device)
            dis_loss = (-(dis_real_loss - dis_fake_loss) + lambdaa * gp)
        ...
        gen.zero_grad()
        genloss = -torch.mean(dis(gen_imgs).view(-1))
        genloss.backward()
        gen_optimizer.step()
        ...
        if i % 200 == 0:
            ...
            save_image(images.data[:64], "wgan_images/%d.png" % batches_done, nrow=8, normalize=True)
        ...
        if epoch % 2 == 0:
            torch.save({
                ...
                }, 'wgan_model/model_epoch_{}.pth'.format(epoch))
...
# plt.show()
anim.save('bitmoji_generated_images_from_wgan.gif', dpi=80, writer='imagemagick')
